...
#import "JPPictureViewController.h"
#import "JPTopic.h"
#import "JPTopicCell.h"

@interface JPPictureViewController ()
@property(nonatomic,strong)AFHTTPSessionManager *manager;
@property(nonatomic,strong)NSMutableArray *pictureTopices;
...
@implementation JPPictureViewController

-(NSMutableArray *)pictureTopices{
    if (!_pictureTopices) {
        _pictureTopices=[NSMutableArray array];
    }
    return _pictureTopices;
}
...
-(void)loadNewWordTopices{
    ...
    [self.manager GET:@"http://api.budejie.com/api/api_open.php" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary *responseObject) {
        ...
        //字典 ---> 模型
        self.pictureTopices=[JPTopic mj_objectArrayWithKeyValuesArray:responseObject[@"list"]];
...
-(void)loadMoreWordTopices{
    ...
    [self.manager GET:@"http://api.budejie.com/api/api_open.php" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary *responseObject) {
        ...
        //字典 ---> 模型
        NSArray *pictureTopices=[JPTopic mj_objectArrayWithKeyValuesArray:responseObject[@"list"]];
        [self.pictureTopices addObjectsFromArray:pictureTopices];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    //若没有帖子数就隐藏上拉刷新控件
    self.tableView.mj_footer.hidden=(self.pictureTopices.count==0);
    return self.pictureTopices.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.topic=self.pictureTopices[indexPath.row];
