...
#import "JPVoiceViewController.h"
#import "JPTopic.h"
#import "JPTopicCell.h"

@interface JPVoiceViewController ()
@property(nonatomic,strong)AFHTTPSessionManager *manager;
@property(nonatomic,strong)NSMutableArray *voiceTopices;
...
@implementation JPVoiceViewController

-(NSMutableArray *)voiceTopices{
    if (!_voiceTopices) {
        _voiceTopices=[NSMutableArray array];
    }
    return _voiceTopices;
}
...
-(void)loadNewWordTopices{
    ...
    [self.manager GET:@"http://api.budejie.com/api/api_open.php" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary *responseObject) {
        ...
        //字典 ---> 模型
        self.voiceTopices=[JPTopic mj_objectArrayWithKeyValuesArray:responseObject[@"list"]];
...
-(void)loadMoreWordTopices{
    ...
    [self.manager GET:@"http://api.budejie.com/api/api_open.php" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary *responseObject) {
        ...
        //字典 ---> 模型
        NSArray *voiceTopices=[JPTopic mj_objectArrayWithKeyValuesArray:responseObject[@"list"]];
        [self.voiceTopices addObjectsFromArray:voiceTopices];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    //若没有帖子数就隐藏上拉刷新控件
    self.tableView.mj_footer.hidden=(self.voiceTopices.count==0);
    return self.voiceTopices.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.topic=self.voiceTopices[indexPath.row];
