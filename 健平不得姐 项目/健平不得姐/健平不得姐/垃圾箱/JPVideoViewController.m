...
#import "JPVideoViewController.h"
#import "JPTopic.h"
#import "JPTopicCell.h"

@interface JPVideoViewController ()
@property(nonatomic,strong)AFHTTPSessionManager *manager;
@property(nonatomic,strong)NSMutableArray *videoTopices;
...
@implementation JPVideoViewController

-(NSMutableArray *)videoTopices{
    if (!_videoTopices) {
        _videoTopices=[NSMutableArray array];
    }
    return _videoTopices;
}
...
-(void)loadNewWordTopices{
    ...
    [self.manager GET:@"http://api.budejie.com/api/api_open.php" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary *responseObject) {
        ...
        //字典 ---> 模型
        self.videoTopices=[JPTopic mj_objectArrayWithKeyValuesArray:responseObject[@"list"]];
...
-(void)loadMoreWordTopices{
    ...
    [self.manager GET:@"http://api.budejie.com/api/api_open.php" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary *responseObject) {
        ...
        //字典 ---> 模型
        NSArray *videoTopices=[JPTopic mj_objectArrayWithKeyValuesArray:responseObject[@"list"]];
        [self.videoTopices addObjectsFromArray:videoTopices];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    //若没有帖子数就隐藏上拉刷新控件
    self.tableView.mj_footer.hidden=(self.videoTopices.count==0);
    return self.videoTopices.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.topic=self.videoTopices[indexPath.row];
