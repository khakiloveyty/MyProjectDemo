...
#import "JPAllViewController.h"
#import "JPTopic.h"
#import "JPTopicCell.h"

@interface JPAllViewController ()
@property(nonatomic,strong)AFHTTPSessionManager *manager;
@property(nonatomic,strong)NSMutableArray *allTopices;
...
@implementation JPAllViewController

-(NSMutableArray *)allTopices{
    if (!_allTopices) {
        _allTopices=[NSMutableArray array];
    }
    return _allTopices;
}
...
-(void)loadNewWordTopices{
    ...
    [self.manager GET:@"http://api.budejie.com/api/api_open.php" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary *responseObject) {
        ...
        //字典 ---> 模型
        self.allTopices=[JPTopic mj_objectArrayWithKeyValuesArray:responseObject[@"list"]];
...
-(void)loadMoreWordTopices{
    ...
    [self.manager GET:@"http://api.budejie.com/api/api_open.php" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary *responseObject) {
        ...
        //字典 ---> 模型
        NSArray *allTopices=[JPTopic mj_objectArrayWithKeyValuesArray:responseObject[@"list"]];
        [self.allTopices addObjectsFromArray:allTopices];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    //若没有帖子数就隐藏上拉刷新控件
    self.tableView.mj_footer.hidden=(self.allTopices.count==0);
    return self.allTopices.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.topic=self.allTopices[indexPath.row];
